...
tellraw @p[tag=sm_player] {"rawtext":[{"translate":"sm.win_epic_self"},{"score":{"objective":"vars","name":"sm_totalwin"}},{"text":"§g$"}]}
tellraw @a[tag=o_smchat,tag=!sm_player] {"rawtext":[{"text":"- §l"},{ "selector": "@p[tag=sm_player]" },{"translate":"sm.win_epic_others"},{"score":{"objective":"vars","name":"sm_totalwin"}},{"text":"§6$§7)§f -"}]}
